...
- `CustomEvent` pattern already used throughout codebase (`voice-nav-mode-changed`, `voice-app-open-mode-changed`) — use namespaced wake events (`voice:wake-activated` / `voice:wake-expired`).
...
const phraseBoundaryMatch = (utterance: string, phrase: string) =>
  new RegExp(`(^|\\s)${phrase.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(\\s|$)`).test(utterance);

if (requireWakePhraseRef.current) {
  const lower = normalize(rawTranscript);
...
- **ADD** cleanup in the unmount `useEffect` (or a new one):
  ...
  ```
- **UPDATE** the `onResult` callback — after final transcript extraction, keep navigation matching first (ungated), then apply wake gate to board commands and agent calls only.
...
```ts
const WAKE_PHRASES = ["listen up", "hey coach", "ok listen up"];
...
- **Wake phrase in the noise**: keep v1 phrase list strict (`listen up`, `hey coach`, `ok listen up`) and expand only after observing real classroom false-positive rates.
